layui.use(['table','jquery','util'], function(){
  ...
  table.render({
    elem : '#sysSleepTable',
    height : 530,
    url : '/skillCompete/sysSleepController/querySleepList',
    limits : [10,20,50],
    page : true, //开启分页
    toolbar: 'default',
    cols : [[ //表头
        {type: 'checkbox', fixed: 'left',width:'5%'},
        {field: 'sleepid', title: 'ID',align : 'center',width:'10%'},
		...
		{field: 'quality', title: '睡眠质量',align : 'center',width:'10%'},
		{field: 'daycount', title: '睡眠时长(小时)',align : 'center',width:'12%'},
		{field: 'des', title: '睡眠描述',align : 'center', width:'21.5%'},
    ]],
    id: 'testReload',
  ...
  $('#bt1').click(function() {
	  ...
	  table.reload('testReload',{
		    url : '/skillCompete/sysSleepController/querySleep',
		    where : {
		    	username : namekey
		    },
		  }); 
...
//监听头工具栏事件
  table.on('toolbar(sysSleepTable)', function(obj){
      ...
      case 'delete':
        if(data.length === 0){
          ...
          layer.confirm('确认删除此行？',{btn:['yes','no']},function(index){
        	  delByName(checkStatus.data[0].sleepid);
          });
        }
      break;
    };
  ...
  function delByName(sleepid) {
		$.ajax({
			url : '/skillCompete/sysSleepController/removeSleep',
